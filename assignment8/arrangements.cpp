 ...
 private:
  ...
  void _Dinner(vector<string>* r, int length) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (r->at(i).front() == _names.at(0)) {
        string t = r->at(i);
        t.at(0) = _names.at(t.size());
        t.push_back(_names.at(0));
        r->push_back(t);
      }
      r->at(i) = r->at(i) + _names.at(length - 1);
    }
    string t0, t1;
    for (int i = 0; i < length - 1; i++) t0 += _names.at(i);
    for (int i = 1; i < length; i++) t1 += _names.at(i);
    t0 = _names.at(t0.size()) + t0;
    t1 += _names.at(0);
    r->push_back(t0);
    r->push_back(t1);
  }
...
void show_partial_result(string testname, vector<string> res, int n) {
  ...
  std::vector<uint64_t> locs{0, res.size() / 3, 2 * res.size() / 3,
                             res.size() - 1};
  std::cout << "\n" << testname << " " << n << "\n";
  for (auto i : locs) {
    std::cout << " res.at(" << i << ") = " << res.at(i) << "\n";
  }
}
...
void standard_tests() {
  ...
  for (auto arr : v) show_result(arr->panel_shuffles(6));

  cout << "\nDinner Shuffles for 6 guests, 3 sets of names.\n";
  for (auto arr : v) show_result(arr->dinner_shuffles(6));
  ...
  while (n < COUNTLIM and t_pc.time() < 0.1) {
    ...
    t_pc.stop();
    cout << std::setw(6) << n << " " << std::setw(6) << pc << "\n";
    n++;
  }
  ...
  while (n < COUNTLIM and t_dc.time() < 0.1) {
    ...
    t_dc.stop();
    cout << std::setw(6) << n << " " << std::setw(6) << dc << "\n";
    n++;
  }
  ...
  cout << "\nLargest panel shuffles performed: " << largest_panel << "\n";
...
void interactive_main() {
  ...
  while (true) {
    ...
    if (asktype == "pc") {
      ...
      for (auto e : h.panel_shuffles(number)) std::cout << e << " ";
      std::cout << "\n";
    } else if (asktype == "dc") {
      ...
      for (auto e : h.dinner_shuffles(number)) std::cout << e << " ";
      std::cout << "\n";
    }
  }
}
