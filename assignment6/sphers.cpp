...
public:
    ...
    {
        //O(n!) function
        return _data[0][0] * _data[1][1] * _data[2][2] + _data[0][1] * _data[1][2] * _data[2][0] + _data[0][2] * _data[1][0] * _data[2][1] - _data[0][2] * _data[1][1] * _data[2][0] - _data[0][0] * _data[1][2] * _data[2][1] - _data[0][1] * _data[1][0] * _data[2][2];
    }
...
int main(int argc, char **argv)
{
    ...
    while (getline(cin, thisBall))
    {
        ...
        double curRadius = stod(ballRadius);

        double posX = stod(bPosX);
        double posY = stod(bPosY);
        double posZ = stod(bPosZ);

        double velX = stod(bVelX);
        double velY = stod(bVelY);
        double velZ = stod(bVelZ);
    ...
    cout << "max collisions " << life << endl;
    for (int i = 0; i < inputedBalls.size(); i++)
    {
        inputedBalls.at(i).PrintOutInfo();
    }
    ...
    while (nextEventTimeSpan > 0)
    {
        ...
        {
            cout << "reflecting " << inputedBalls.at(eventBallA)._name << endl;
        }

        //===========================================================================
        for(int i=0;i<inputedBalls.size();i++){
            if(inputedBalls.at(i)._life<=0)continue;
            inputedBalls.at(i).PrintOutInfo();
        }
        //========================================================================
        cout << "energy: " << GetSystemEnergy(inputedBalls) << endl;
        ...
        inputedBalls.at(eventBallA)._life--;
        if(eventBallB!=eventBallA)inputedBalls.at(eventBallB)._life--;
        //===========================================================================
